...
public class TranscriptFetcher
{
    ...
    // Function to export subtitles asynchronously
    public async Task<string> ExportSubtitlesAsync(string transcriptId, string format, string apiToken)
    {
...
public class Program
{
    public static async Task Main(string[] args)
    {
        ...
        try
        {
            ...
            string subtitles = await transcriptFetcher.ExportSubtitlesAsync(transcript.id.ToString(), subtitleFormat, apiToken);

            // Print the subtitles to the console
            Console.WriteLine($"Subtitles ({subtitleFormat.ToUpper()}):\n{subtitles}");
        }
        catch (Exception ex)
        {
