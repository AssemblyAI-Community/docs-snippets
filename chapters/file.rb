...
# Print the chapters
puts "Chapters:"
transcript["chapters"].each_with_index do |chapter, index|
  puts "\nChapter #{index + 1}:"
  puts "  Summary: #{chapter['summary']}"
  puts "  Headline: #{chapter['headline']}"
  puts "  Gist: #{chapter['gist']}"
  puts "  Start: #{chapter['start']}"
  puts "  End: #{chapter['end']}"
end
